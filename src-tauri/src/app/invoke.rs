...
#[command]
pub async fn update_theme_mode(app: AppHandle, _mode: String) {
    let _window = app.get_webview_window("pake").unwrap();
    #[cfg(target_os = "macos")]
    {
        let theme = if _mode == "dark" {
            Theme::Dark
        } else {
            Theme::Light
        };
        let _ = _window.set_theme(Some(theme));
    }
}
